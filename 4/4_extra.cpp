...
//const int WIN_COLS = 5;
//const int MY_COLS = 8;
//const int LINE_HEADER_LEN = 8;
...
int main(int argc, char *argv[]){
    ...
    int num_cards_rep[n_lines]; //Zeros as default
    string line;
    string win_numbers, my_numbers;
    int prev_ret = 0, next_ret = 0;

    //Better to cure us in health (efficent C way)...
    memset(num_cards_rep, 0, sizeof(num_cards_rep));
    ...
    int result = 0;
    for(int i = 0; i < n_lines; i++){
        int line_points_rep = 0;
        //Check number of wins taking in mind reps. Sry for the triple nested for
        ...
        //Update repeats
        //I'm sure there is a mathematical expression to skip this
        for(int card = 0; card < num_cards_rep[i] + 1; card++){
          for (int v = 1; v <= line_points_rep; v++) {
            if (i + v < n_lines)
              num_cards_rep[i + v]++;
          }
        }
        ...
        cout << "Card: " << i << " has "
             << line_points_rep / (num_cards_rep[i] + 1) << " coincididences"
             << " is repeated " << num_cards_rep[i] + 1 << " times"
             << " clones next: " << line_points_rep << " cards" << endl;

        result += num_cards_rep[i] + 1;
    }
